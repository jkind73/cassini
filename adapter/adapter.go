...
func (f *Factory) SystemInfo() coreif.SystemInfo {
	return coreif.SystemInfo{
		...
		CoreOptions: []coreif.CoreOption{
			...
			{
				Key:         "scale_factor",
				Label:       "Internal 3D Resolution",
				Description: "VDP1 internal resolution scale factor",
				Type:        coreif.CoreOptionSelect,
				Default:     "1x",
				Values:      []string{"1x", "2x", "3x", "4x"},
				Category:    coreif.CoreOptionCategoryVideo,
			},
			{
				Key:         "texture_filtering",
				Label:       "Texture Filtering",
				Description: "Bilinear sub-texel filtering for 3D sprites and polygons",
				Type:        coreif.CoreOptionBool,
				Default:     "false",
				Category:    coreif.CoreOptionCategoryVideo,
			},
			{
				Key:         "aspect_ratio",
				Label:       "Aspect Ratio",
				Description: "Viewport aspect ratio mode",
				Type:        coreif.CoreOptionSelect,
				Default:     "4:3",
				Values:      []string{"4:3", "16:9", "integer", "stretch"},
				Category:    coreif.CoreOptionCategoryVideo,
			},
			{
				Key:         "display_shader",
				Label:       "Display Shader",
				Description: "Post-processing CRT shader filter",
				Type:        coreif.CoreOptionSelect,
				Default:     "normal",
				Values:      []string{"normal", "crt"},
				Category:    coreif.CoreOptionCategoryVideo,
			},
			{
				Key:         "upscaler_filter",
				Label:       "Upscaler Filter",
				Description: "Presentation upscaling filter",
				Type:        coreif.CoreOptionSelect,
				Default:     "nearest",
				Values:      []string{"nearest", "bilinear", "bicubic", "hq2x", "hq4x", "xbrz"},
				Category:    coreif.CoreOptionCategoryVideo,
			},
			{
				Key:         "crt_bloom",
				Label:       "CRT Bloom",
				Description: "CRT phosphor bloom glow effect",
				Type:        coreif.CoreOptionBool,
				Default:     "false",
				Category:    coreif.CoreOptionCategoryVideo,
			},
			{
				Key:         "crt_curvature",
				Label:       "CRT Curvature",
				Description: "CRT curved glass distortion effect",
				Type:        coreif.CoreOptionBool,
				Default:     "false",
				Category:    coreif.CoreOptionCategoryVideo,
			},
